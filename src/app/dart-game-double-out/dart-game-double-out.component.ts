...
export class DartGameDoubleOutComponent implements OnInit {
  ...
  calcScore(thrownNumber: string) {
    ...
    if (
      (this.gameData[this.currentPlayer].score === 0 &&
        multiplierFactor === 2) ||
      (this.gameData[this.currentPlayer].score === 0 && parseInt(number) === 50)
    ) {
      this.doubleOut = true;
    }
    localStorage.setItem('gameData', JSON.stringify(this.gameData));
  ...
  undoLastAction() {
    ...
    if (currentPlayer.firstDart != '-') {
      ...
      if (currentPlayer.firstDart === '-') {
        ...
        } else if (!this.gameData[this.currentPlayer - 1].isActive && currentPlayer.round === 1) {
          this.undoLastActionEnabled = false;
        } else if (!this.gameData[this.currentPlayer - 1].isActive && currentPlayer.round > 1 && this.lastActivePlayer && currentPlayer.firstDart === "-") {
          this.undoLastActionEnabled = false;
        }
      }
